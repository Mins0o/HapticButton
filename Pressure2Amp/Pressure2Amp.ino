# define DURATION 12000 // max 65535
...
int8_t* wave_points = WAVE_;
uint8_t loop_point = LOOP_;
...
ISR(TIMER2_OVF_vect){
  if(play_){
    float saw_enveloped = wave_points[i++]*(long)(DURATION-TCNT1)/DURATION*pressure_scaler+128;
    OCR2A = (uint8_t)saw_enveloped;
    if (i>loop_point){
      i=0;
    }
    if(TCNT1 < 65 || TCNT1 > DURATION-65){ // Racing resolving threshold_Found by trial and error1
      play_ = false;
      OCR2A=128;
    }
  }
