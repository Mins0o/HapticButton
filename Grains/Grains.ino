...
void loop(){
  ...
  if(~PIND & 0b10000000){
    ...
    pres = (long(pres)*pres)/1024;
  }
  grain_interval = (~PIND & 0b00011100) * 12.5 - 1;
  if (pull_next){
    if(PIND & 0b00100000){ // When not connected to ground
     random_ = random(-50,51)*int(grain_interval)/100;
    }else{
      random_ = 0;
    }
    pull_next = false;
