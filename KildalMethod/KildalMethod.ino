...
uint16_t duration = 30000; //max 65535
...
ISR(TIMER2_OVF_vect){
  if(play_){
    ...
    float saw_enveloped = sin500_zn[i]*(long)(duration-TCNT1)/duration+128;
    OCR2A = (uint8_t)saw_enveloped;
    i = i+1;
    if (i>61){
      i=0;
    }
    if(TCNT1 < 52 || TCNT1 > duration-52){ // Racing resolving threshold_Found by trial and error
      play_ = false;
    }
  }
}
